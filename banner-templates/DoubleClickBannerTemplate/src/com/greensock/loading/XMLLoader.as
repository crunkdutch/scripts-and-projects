		...
		protected function _changeQueueListeners(add:Boolean):void {
			if (_loadingQueue != null) {
				...
				if (add && this.vars.integrateProgress != false) {
					_loadingQueue.addEventListener(LoaderEvent.COMPLETE, _completeHandler, false, -1, true);
					_loadingQueue.addEventListener(LoaderEvent.PROGRESS, _progressHandler, false, -1, true);
					_loadingQueue.addEventListener(LoaderEvent.FAIL, _failHandler, false, -1, true);
					for (p in _listenerTypes) {
						if (p != "onProgress" && p != "onInit") {
							_loadingQueue.addEventListener(_listenerTypes[p], _passThroughEvent, false, -1, true);
						}
					}
				} else {
