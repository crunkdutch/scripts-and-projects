 ...
 */	
	public class LoaderMax extends LoaderCore {		
		/** @private **/
		public static const version:Number = 1.79;
	...
	trace("finished loading " + loader.numChildren + " loaders.");
		...
		public function insert(loader:LoaderCore, index:uint=999999999):LoaderCore {
			...
			if (this != _globalRootLoader) {
				loader.addEventListener(LoaderEvent.PROGRESS, _progressHandler, false, -1, true); //use -1 so that if the user adds an event listener, it gets called before LoaderMax is notified. Otherwise bubbling behavior doesn't go in the proper order.
				loader.addEventListener("prioritize", _prioritizeHandler, false, -1, true);
				for (var p:String in _listenerTypes) {
					if (p != "onProgress" && p != "onInit") {
						loader.addEventListener(_listenerTypes[p], _passThroughEvent, false, -1, true);
					}
				}
			}
			loader.addEventListener("dispose", _disposeHandler, false, -1, true);
			_cacheIsDirty = true;
