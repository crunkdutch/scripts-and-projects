/**
 * PointRoll Video Control Loader/Buffer - Progress Bar Combo
 * 
 * Shows a progress bar for the playback
 * 
 * @class 		PRVCLoaderProgress
 ...
 package PointRollAPI_AS3.components.video.controls  
{
	...
	/** @private **/
	public class PRVCLoaderProgress extends PointRollAPI_AS3.components.video.controls.PRVCUIProgress {
		
		//internal use, needed for component structure
		static var symbolName:String = "PRVCLoaderProgress";
    	static var symbolOwner:Object = PointRollAPI_AS3.components.video.controls.PRVCLoaderProgress;
		...
		private var maskBar:MovieClip;
		
		private var dragDriver:Boolean;
		
		function PRVCLoaderProgress() {
			super();
			dragDriver = false;
		...
		private function getChildren(event:Event):void {
			...
			maskBar = MovieClip(getChildByName("maskBarOnStage"));
			init();
		}
		...
		*/
		...
		/**
		*	Shows the for the loaded amount, receives a number between 0-100
		*/
		public function showLoaded(progress:Number):void {
			if(progress>0) maskBar.width = maxWidth * progress;
		}
		...
		*/
		...
		private function onProgress(e:PrProgressEvent):void {
			...
			if(!prVideoDisplay.isStreaming()) {
				...
				//STREAMING MODE
				p = e.currentTime / e.totalTime; //+e.bufferSecs
			}
			showLoaded(p);
		}
