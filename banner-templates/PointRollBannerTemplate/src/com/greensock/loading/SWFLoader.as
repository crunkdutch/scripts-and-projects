		...
		protected function _changeQueueListeners(add:Boolean):void {
			if (_queue != null) {
				...
				if (add && this.vars.integrateProgress != false) {
					_queue.addEventListener(LoaderEvent.COMPLETE, _completeHandler, false, -1, true);
					_queue.addEventListener(LoaderEvent.PROGRESS, _progressHandler, false, -1, true);
					_queue.addEventListener(LoaderEvent.FAIL, _failHandler, false, -1, true);
					for (p in _listenerTypes) {
						if (p != "onProgress" && p != "onInit") {
							_queue.addEventListener(_listenerTypes[p], _passThroughEvent, false, -1, true);
						}
					}
				} else {
